...
const ProductPagePromoBar = props => {
  ...
  const productsListItems = products.map(product => {
    const { pictures, name, price, id } = product;
    return (
      <li key={id} className="column is-6-mobile is-3-desktop product-page-promo__item">
  ...
  return (
    <div className="product-page-promo">
      ...
      <ul className="columns is-mobile is-multiline is-centered has-text-centered product-page-promo__list">
        {productsListItems}
      </ul>
    </div>
  );
