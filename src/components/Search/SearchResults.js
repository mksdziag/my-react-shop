...
const SearchResults = props => {
	...
	if (results.length > 0) {
		searchResultsItems = results.map(product => {
			const { _id, name, price, pictures, category, brand } = product;

			return (
				<li className="is-primary has-text-black is-fullwidth search-results__item" key={_id}>
					<Link className="has-text-grey" to={`/product/${_id}`}>
