	...
	return (
		<div className={'card product-card ' + additionalClasses}>
			<div className="card-image product-card__image-holder">
				<Link to={`/product/${_id}`}>
					<figure className="image product-card__figure ">
						...
						<WishListButton itemId={_id} additionalClasses="product-card__wishlist-add " />
					</figure>
				</Link>
			</div>
			<div className="card-content product-card__info">
				<header>
					<Link to={`/product/${_id}`} className="title is-size-5 product-card__name">
						{name}
					</Link>
				</header>
			...
			<footer className="product-card__actions">
				...
				<Link to={`/product/${_id}`} className="button is-fullwidth is-primary is-outlined">
